...
- **Sync whitelist glob + retired directories (ADR 002, 2026-09-26)**: a `PATHS` entry may be a FILE GLOB with a literal `harness-` prefix (`.omp/rules/harness-*.md`) — expansion is NUL-framed, never crosses `/`, the consumer-side directory must resolve inside the checkout through real directories only (`glob_dir_ok`, ancestors included), matched symlinks are unlinked without following, a directory bearing a matched name is never written into, source names must be plain `[A-Za-z0-9._/-]`, and the provenance tree/manifest are built from the same expanded list via `git_literal` (`--literal-pathspecs` with the four `GIT_*_PATHSPECS` cleared). A directory the harness used to ship (`RETIRED_DIRS=(rules)`) is retired by step 7c: only files whose `hash-object --no-filters` blob equals the PREVIOUS synced tree's blob are removed; edited/added files, a missing previous tree, or a previous tree without that directory yield an advisory and no deletion. Fixtures: `tests/harness-sync.test.mjs` `ADR002*`.
