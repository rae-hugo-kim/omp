...
그 외 (탐색/조사/구현)?
  → 직접 처리 또는 빌트인 에이전트 사용 (explore, task, OMC executor 등)
```
...
- **운영 팁**: 완료된 서브에이전트는 잠시 idle로 살아있다(이후 parked). reviewer/verifier 후속 질의는 재스폰 대신 `irc`로 해당 에이전트에 이어가면 컨텍스트를 보존한다.
...
- **근거**: 전 프로젝트·전 기간 세션 로그 실측에서 위임 0회 (같은 기간
  ...
  않음). 외부 조사·DB·리팩터링 작업은 빌트인(explore/task/web_search)과
  직접 MCP 호출이 흡수했다.
