...
- Log: `.omp/harness-state/hook-debug.log` (written only when `HARNESS_DEBUG` is set)
- Reads: `docs/reviews/review-<today>*.md` (diff-hash coverage; second-perspective evidence = a MEASURED `models:` line naming >=2 distinct model families — the reviewer writes it only after transcript-verifying the adversary's resolved family; thread/session id fields (`codex-thread:` 등) are NOT evidence — **or** `human-reviewed-by:` + a `Verdict:` on the PASS whitelist (`PASS` / `PASS WITH NOTES` only)), `docs/harness/review-skip` (audited override: `reason:` / `approved-by:` / `diff-hash:` fields — commit-diff-bound, consumed on use, audited)
