...
- **Helper modules** (imported by other gates or by the extension, never spawned themselves): `git-commit-detect` (shared `isGitCommit` detector imported by `index.ts` and used by `commit-gates`, `acceptance-gate`, `backpressure-gate`, `review-gate`), `risk-assess` (risk classification imported by `review-gate` and `backpressure-gate`), `backpressure-patterns` (shared by `backpressure-tracker` and `backpressure-failure-tracker`), `read-path` (imported by `index.ts`: `readTarget` strips a read selector / filters URLs so read-tracker logs the bare path context-gate compares against, and `resolvedAstEditFiles` extracts the written files from an `ast_edit` `resolve` apply result).
- **Standalone advisory / lifecycle gates** (wired in `index.ts`, non-blocking): `destructive-guard` (`tool_call`: bash, scans every command), `mcp-gate` (advisory notice on `mcp__*` tool calls), `backpressure-invalidator` (`tool_result`: edit/write; for `ast_edit` it runs on the PREVIEW as a safety fallback AND on the real `resolve` apply — marks verification state stale), `harness-version-check` (`session_start` with the 24h default window; ALSO re-run agent-facing with a 1h `max_age_ms` window at `before_agent_start` — merged into the `harness-reminder` message — and after a successful `git commit` on `tool_result`, where drift text is appended to the commit's tool result. Failed probes write a short-lived failure marker so frequent callers back off instead of re-stalling on a dead network).
