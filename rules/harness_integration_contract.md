...
- **Standalone advisory / lifecycle gates** (wired in `index.ts`, non-blocking): `destructive-guard` (`tool_call`: bash, scans every command), `mcp-gate` (advisory notice on `mcp__*` tool calls), `backpressure-invalidator` (`tool_result`: edit/write; for `ast_edit` it runs on the PREVIEW as a safety fallback AND on the real `resolve` apply — marks verification state stale), `harness-version-check` (`session_start`).
