...
- **fix(harness)**: review-gate 사이드카 스캔 윈도우를 오늘+어제(로컬 날짜)로 확장 (#48-2) — 자정 직전에 쓴 유효한 리뷰 증거(해시 일치)가 자정 뒤 커밋에서 `review-<today>*` 필터에 걸러져 무효가 되던 마찰. 관련성은 해시 바인딩이 보장하므로 날짜는 스캔 범위만 한정: 어제+해시 일치 → PASS, 어제 covering FAIL → 여전히 BLOCK, 그제 → 윈도우 밖. `MAX_SIDECARS` 캡은 이틀 합산. 테스트 3건 교체(`review-gate.test.mjs`).
- **feat(harness)**: 프로젝트 고유 검증 명령 등록 `docs/harness/verify-commands.json` (#48-3) — `{"test":["bash bpy/bench/run.sh"],"lint":[…],"build":[…]}` 리터럴 접두사(정규식 아님, ASCII 공백 토큰 경계, 200자 이하)를 `classifyVerification(command, cwd)`가 내장 VERIFY 표보다 먼저 대조. backpressure-tracker·failure-tracker·breadcrumb-tracker 세 소비자가 cwd를 넘김. 파일 없음·JSON 오류·비문자열 항목은 항목 단위로 무시(기본 표만); 런처·패키지 매니저·VCS·실행 모드 단어(`run`/`exec`/`.`/`command`)·옵션·옵션의 맨 값만으로 된 등록(`bash`, `/usr/bin/bash`, `python3.12`, `node -e`, `bash +e`, `npm run`, `npm --prefix web run`, `git -C /tmp`…; basename·대소문자·버전 접미 무시, 따옴표 포함 등록은 거부)은 모든 명령을 PASS로 만들므로 거부하고 구체적 러너 토큰(`npm run bench`, `bash bpy/bench/run.sh`, `python3 -m tests.bench`)을 요구(3-pass 리뷰 5라운드 high); 트림은 ASCII만(NBSP 경계 방어); 파일은 64KiB·정규 파일·`O_NONBLOCK`으로 읽어 FIFO가 트래커를 멈추게 하지 못함. 벤치 러너처럼 내장 표에 없는 검증을 실제로 돌려도 "No build/test verification"으로 막혀 `backpressure-skip`에 의존하던 마찰 해소. 차단 메시지에 등록 파일 안내. 테스트 6건(토큰 경계·NBSP 양끝(명령·등록 양쪽)·내장 우선순위 relabel·bash -c 재귀·무효 파일 6형·런처/경로/옵션/실행모드 거부·FIFO(writer 유무)/디렉터리/심링크·항목 단위 검증).
